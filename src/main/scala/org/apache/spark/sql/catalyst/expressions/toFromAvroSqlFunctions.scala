...
/** Converts a binary column of Avro format into its corresponding Catalyst
  * value. This is a thin wrapper over the [[AvroDataToCatalyst]] class to
...
case class FromAvro(
  ...
  override def checkInputDataTypes(): TypeCheckResult = {
    ...
    val optionsCheck = options.dataType match {
      case MapType(StringType, StringType, _) | MapType(NullType, NullType, _) |
          _: NullType if options.foldable =>
        None
      case _ =>
        Some(
          TypeCheckResult.TypeCheckFailure(
  ...
  override def replacement: Expression = {
    ...
    val constructor =
      Utils
        .classForName("org.apache.spark.sql.avro.AvroDataToCatalyst")
        .getConstructors()
        .head
...
/** Converts a Catalyst binary input value into its corresponding AvroAvro
  * format result. This is a thin wrapper over the [[CatalystDataToAvro]] class
...
case class ToAvro(child: Expression, jsonFormatSchema: Expression)
  ...
  override def replacement: Expression = {
    ...
    val constructor =
      Utils
        .classForName("org.apache.spark.sql.avro.CatalystDataToAvro")
        .getConstructors()
        .head
...
object RegisterFunction {
  ...
  val fromAvroFunctionName = "from_avro"
}

import org.apache.spark.sql.SparkSessionExtensions
class AvroExtensions extends Function1[SparkSessionExtensions, Unit] {
  import RegisterFunction._
  override def apply(extensions: SparkSessionExtensions): Unit = {
    val (name, (info, builder)) = expression[FromAvro](fromAvroFunctionName)
